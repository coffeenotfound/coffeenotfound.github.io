module Jekyll
	...
	module StopWordFilter
		def stopwordfilter(input)
			#if input.is_a? Array == false
			#	raise "stopwordfilter excepts array"
			#end
			...
			# iterate over input
			input.each { |x|
				if not Jekyll::STOP_WORDS.member?(x)
					output.push(x)
				end
			}
...
Liquid::Template.register_filter(Jekyll::StopWordFilter)